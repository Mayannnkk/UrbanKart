...
export default function Filter() {
    ...
    const newcategoryarray = products.map((item) => item.category).filter((value, index, current_value) => current_value.indexOf(value) === index)
    const newprice = products.map((item) => item.price).filter((value, index, current_value) => current_value.indexOf(value) === index)
            ...
            <div className='container mx-auto px-4 mt-5'>
                <div className="p-5 rounded-lg bg-gray-100 drop-shadow-xl border border-gray-200"
                    ...
                    <div className="flex items-center justify-between mt-4">
                        ...
                        <button onClick={refreshPage} className="px-4 py-2 bg-gray-50hover:bg-gray-200 text-gray-800 text-sm font-medium rounded-md" style={{ color: mode === 'dark' ? 'white' : '' }}>
                            Reset Filter
                        </button>
                    </div>
                    ...
                    <div>
                        <div className="grid grid-cols-2 md:grid-cols-3 xl:grid-cols-4 gap-4 mt-4">
                            <div>
                                <span className='px-3 py-5 text-sm color-black'>Category</span>
                                <select value={filterType} onChange={(e) => setFilterType(e.target.value)} className="px-4 py-3 w-full rounded-md bg-gray-50 border-transparent outline-0 focus:border-gray-500 focus:bg-white focus:ring-0 text-sm" style={{ backgroundColor: mode === 'dark' ? 'rgb(64 66 70)' : '', color: mode === 'dark' ? 'white' : '', }}>
                                    {newcategoryarray.map((item, index) => {
                                        return (
                            ...
                            <div>
                                <span className='px-3 py-5 text-sm color-black'>Price</span>
